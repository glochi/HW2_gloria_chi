...
gpl.dt[(data.table(dbGetQuery(geo_con,"SELECT * FROM gse_gpl;"), key=c ("gse", "gpl")))[(data.table(dbGetQuery(geo_con,"SELECT * FROM gse;"), key="gse"))[(summary %like% 'HCV'| title %like%
...
```{r}
#Using brackets
(data.table(dbGetQuery(geo_con,"SELECT * FROM gpl;"), key="gpl"))[gse_gpl.dt[gse.dt[(summary %like% 'HCV'| title %like%
'HCV') & contact %like% ' Yale ', list(gse, title)], list(gpl, i.gse,
title), nomatch=0], list(gpl, i.gse, title, manufacturer, description),
nomatch=0][manufacturer %like% 'Illumina']
